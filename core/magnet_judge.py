...
# 「极高」的评论数门槛。**暂定值**，等全盘库存跑完看分布再定
# （ROADMAP 第 8 节待定事项 1）。
COMMENTS_FOR_TOP = 50

# 正确磁力条数的门槛。
#
# 依据：library.db 实测 252 部有磁力的片，按**正确**磁力条数分布定
# （不是原始条数 —— 原始分布会把挂错的磁力也算进来，门槛会定低）。
#
# 实测分布（2026-09-23，判定器正确率 99.7%）：
#     0 条      2 部 ( 0.8%)
#     1-2 条    4 部 ( 1.6%)
#     3-4 条    7 部 ( 2.8%)
#     5-9 条   47 部 (18.7%)
...
#     50+ 条    7 部 ( 2.8%)
#
# ⚠️ **关键事实**：>=3 条覆盖 97.6% —— 「高」档作为**筛选**几乎没有区分力。
# 它作为**删除闸门**（要保守，只要证据存在就放行 + D11 第二条兜底）是合适的，
# 但**不要**把它当成「这部片证据很强」的信号。要后者得用 10 条（覆盖 76.2%）。
#
# 结论：门槛值保持 3（保守优先），但**新增 strong 一档供展示/排序用**，
# 不参与删除门控。
MAGNETS_FOR_HIGH = 3        # >=3 条 = 高（删除闸门）
MAGNETS_FOR_STRONG = 10     # >=10 条 = 证据强（仅展示/排序，不做门控）
...
def tier_of(correct, comments=None):
    ...
    「高」与「极高」的磁力条件相同，只有评论数不同 —— 这是有意的：
    极高表示「证据强 **且** 有讨论度」，不是「磁力更多」。
    """

    c = _as_int(correct, 0) or 0

    if c < MAGNETS_FOR_HIGH:
        return "低" if c >= 1 else "极低"

    # 到这里 c >= MAGNETS_FOR_HIGH。评论门槛只在这一支上生效。
    comments_n = _as_int(comments)

    if comments_n is not None and comments_n >= COMMENTS_FOR_TOP:
        return "极高"
